...
-- Jalon "suppression de matieres + anti-doublons" (signalement de Phil : pas
-- de doublons de matieres ni de chapitres au sein d'une meme matiere).
-- L'appli verifie deja cote client avant de creer une matiere/un chapitre ;
-- ces index uniques (insensibles a la casse) empechent un doublon meme en
-- cas de double-clic ou de creation concurrente par deux comptes.
create unique index if not exists matieres_nom_unique_ci on matieres (lower(nom));
create unique index if not exists chapitres_nom_unique_ci on chapitres (matiere_id, lower(nom));
...
create table documents (
  ...
  archive boolean not null default false, -- Jalon "répertoire d'archivage" (signalement de Phil) : true une fois archivé manuellement depuis l'onglet Documents — le document sort de l'onglet Documents et apparait dans l'onglet Archives, sans être supprimé
...
-- pouvoir lui pousser des notifications même quand l'app/l'onglet est fermé
-- (voir supabase/push_notifications.sql pour les déclencheurs).
...
-- Jalon "correction du bug 2e test IA sur le meme chapitre" (signalement de
-- Phil : un 2e test genere par IA dans le meme chapitre affichait la note et
-- le corrige de l'ancien test au lieu d'un questionnaire vierge). Chaque
-- devoir de type test est desormais rattache explicitement au test qu'il
-- concerne, au lieu d'etre resolu par chapitre (ambigu des qu'un chapitre
-- contient plusieurs tests).
alter table devoirs add column if not exists test_id uuid references tests(id) on delete set null;

-- Jalon "telephone obligatoire a la demande de compte" (signalement de Phil :
-- il faut pouvoir documenter le numero de telephone des la demande de
-- creation de compte, pas seulement l'ajouter apres coup depuis Admin >
-- Comptes -- voir Jalon "icones appel/SMS" plus haut qui avait deja ajoute
-- comptes.telephone sans le documenter ici). demandes_comptes.telephone est
-- repris tel quel sur la ligne comptes creee a la validation (voir
-- app/api/demandes/[id]/valider/route.js).
alter table demandes_comptes add column if not exists telephone text;
alter table comptes add column if not exists telephone text;

-- Jalon "personnalisation de l'espace Enfant" (signalement de Phil : rendre
-- l'application plus attractive pour une adolescente, comme le streak et le
-- minuteur focus) : couleur d'accent choisie librement par l'enfant parmi
-- une palette fixe (soulignement de l'onglet actif, bouton Démarrer du
-- minuteur Focus...). Valeur nulle = couleur par défaut de l'application
-- (royalblue #4169E1). Modifiable uniquement via app/api/mon-compte, jamais
-- en écriture directe côté client — donc pas besoin d'une nouvelle policy
