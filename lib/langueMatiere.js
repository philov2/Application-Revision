...
export function consigneLangue(nomMatiere) {
  ...
  return `Cette matière porte sur l'apprentissage de la langue étrangère suivante : ${langue}. Rédige les consignes, instructions et explications en français (l'enfant doit comprendre facilement ce qu'il doit faire). En revanche, tout le contenu linguistique proprement dit — mots de vocabulaire, phrases, textes à lire/traduire/compléter, dialogues, réponses attendues dans la langue — doit être en ${langue}, jamais en français ni dans une autre langue étrangère. Ne mélange jamais deux langues étrangères différentes : si la matière est ${langue}, tout le vocabulaire et les phrases doivent être exclusivement en ${langue} (pas un mot d'anglais dans un exercice d'allemand, pas un mot d'allemand dans un exercice d'anglais, etc.).`;
}
