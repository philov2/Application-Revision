...
// possibles d'un message. compteIdAExclure permet d'exclure l'auteur
// (on ne s'envoie pas un message a soi-meme).
export async function chargerMembresFamille(enfantId, compteIdAExclure) {
if (!enfantId) return [];
const [{ data: enfant }, { data: parents }, { data: soutiens }] = await Promise.all([
supabase.from("comptes").select("id, nom, role").eq("id", enfantId).maybeSingle(),
supabase.from("liens_parent_enfant").select("parent:comptes!parent_id (id, nom, role)").eq("enfant_id", enfantId),
supabase.from("liens_soutien").select("soutien:comptes!soutien_id (id, nom, role)").eq("enfant_id", enfantId),
]);

const membres = [];
const vus = new Set();
function ajouter(compte) {
if (compte && !vus.has(compte.id)) {
vus.add(compte.id);
membres.push(compte);
}
}
ajouter(enfant);
(parents || []).forEach((l) => ajouter(l.parent));
(soutiens || []).forEach((l) => ajouter(l.soutien));

return membres.filter((m) => m.id !== compteIdAExclure);
}
