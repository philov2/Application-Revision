...
// Empeche un appel bloque cote fournisseur de monopoliser tout le temps
// disponible (plafond de 60s sur le plan Vercel Hobby) : au-dela du delai,
// on abandonne cet appel pour laisser une chance au fournisseur de secours.
async function fetchAvecDelai(url, options, delaiMs) {
  ...
  try {
    ...
    if (err.name === "AbortError") {
      throw new Error(`delai depasse apres ${Math.round(delaiMs / 1000)}s`);
    }
    throw err;
  } finally {
