...
export async function chargerDevoirs(enfantId) {
  const { data, error } = await supabase
    .from("devoirs")
    .select("id, enfant_id, type, date_echeance, statut, date_realisation, matiere_id, chapitre_id, matiere:matieres(nom), chapitre:chapitres(nom), createur:comptes!cree_par(nom, role), reponses_exercices(id, photo_url, note, commentaire, date_soumission)")
