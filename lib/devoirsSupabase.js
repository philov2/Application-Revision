...
function mapDevoir(row) {
  return {
    ...
    document: row.document ? { id: row.document.id, nom: row.document.nom, type: row.document.type, fichierUrl: row.document.fichier_url } : null,
  };
}
...
export async function chargerDevoirs(enfantId) {
  const { data, error } = await supabase
    .from("devoirs")
    .select("id, enfant_id, type, date_echeance, statut, date_realisation, matiere_id, chapitre_id, document_id, matiere:matieres(nom), chapitre:chapitres(nom), createur:comptes!cree_par(nom, role), reponses_exercices(id, photo_url, note, commentaire, date_soumission), document:documents(id, nom, type, fichier_url)")
...
export async function creerDevoir({ enfantId, matiereId, chapitreId, documentId, type, dateEcheance, creePar }) {
...
export async function modifierDevoir(devoirId, { matiereId, chapitreId, documentId, type, dateEcheance }) {
  const { error } = await supabase
    .from("devoirs")
    .update({
      matiere_id: matiereId,
