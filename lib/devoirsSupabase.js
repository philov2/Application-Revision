...
function mapDevoir(row) {
  return {
    ...
    testId: row.test_id,
    flashcardsId: row.flashcards_id,
    matiere: row.matiere?.nom || "",
    couleur: row.matiere?.couleur || null,
...
export async function chargerDevoirs(enfantId) {
  const { data, error } = await supabase
    .from("devoirs")
    .select("id, enfant_id, type, titre, date_echeance, statut, date_realisation, matiere_id, chapitre_id, document_id, test_id, flashcards_id, matiere:matieres(nom, couleur), chapitre:chapitres(nom), createur:comptes!cree_par(nom, role), reponses_exercices(id, photo_url, fichiers_urls, note, commentaire, date_soumission), document:documents(id, nom, type, fichier_url, genere_par_ia, format, corrige_statut, corrige_erreur)")
...
export async function creerDevoir({ enfantId, matiereId, chapitreId, documentId, testId, flashcardsId, titre, type, dateEcheance, creePar }) {
  const { error } = await supabase.from("devoirs").insert({
    ...
    test_id: testId || null,
    flashcards_id: flashcardsId || null,
    titre: titre || null,
    type,
