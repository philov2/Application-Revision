// Calcule les indicateurs des cases resume des tableaux de bord (Parent,
// Enfant, Soutien) a partir d'une liste de devoirs.
//
// IMPORTANT : la "semaine" scolaire va du SAMEDI au VENDREDI, et non du
// lundi au dimanche (semaine calendaire classique). Un devoir est considere
// "en retard de plus d'une semaine" si son echeance est anterieure au samedi
// qui a ouvert la semaine derniere (donc absente a la fois de la semaine en
// cours et de la semaine derniere).

// Renvoie le samedi (00:00) qui ouvre la semaine contenant `date`.
function debutSemaine(date) {
  const d = new Date(date);
  d.setHours(0, 0, 0, 0);
  const jour = d.getDay(); // 0 = dimanche ... 6 = samedi
  const decalage = (jour + 1) % 7; // samedi -> 0, dimanche -> 1, ..., vendredi -> 6
  d.setDate(d.getDate() - decalage);
  return d;
}

export function calculerStatsDevoirs(devoirs, aujourdHui = new Date()) {
  const today = new Date(aujourdHui);
  today.setHours(0, 0, 0, 0);
  const todayStr = today.toISOString().slice(0, 10);

  const debutSemaineEnCours = debutSemaine(today);
  const seuilRetardSemaine = new Date(debutSemaineEnCours);
  seuilRetardSemaine.setDate(seuilRetardSemaine.getDate() - 7); // debut de la semaine derniere

  let aFaireAujourdhui = 0;
  let totalAFaire = 0; // nombre total de devoirs non faits (toutes echeances confondues) ; voir "Total des devoirs a faire"
  let enRetard = 0;
  let realisesDansLesDelais = 0;
  let retardPlusUneSemaine = 0;

  for (const d of devoirs || []) {
    if (!d.echeance) continue;
    const echeance = new Date(d.echeance + "T00:00:00");

    if (d.statut === "fait") {
      const dateRealisation = d.date_realisation ? new Date(d.date_realisation) : null;
      if (!dateRealisation || dateRealisation <= echeance) {
        realisesDansLesDelais++;
      }
      continue;
    }

    totalAFaire++;

    if (d.echeance === todayStr) {
      aFaireAujourdhui++;
    }
    if (echeance < today) {
      enRetard++;
      if (echeance < seuilRetardSemaine) {
        retardPlusUneSemaine++;
      }
    }
  }

  return { aFaireAujourdhui, totalAFaire, enRetard, realisesDansLesDelais, retardPlusUneSemaine };
}

// Devoirs de type "exercice" dont la réponse a été envoyée par l'enfant
// (statut passé automatiquement à "fait" lors de l'envoi) mais pas encore
// corrigés par le parent ou le soutien (aucune note donnée). Affichés dans
// une section dédiée "À corriger" (Parent/Soutien) ou "Fait - en attente de
// correction" (Enfant), séparée des "Devoirs faits" (déjà corrigés) et des
// "Devoirs à faire".
export function filtrerDevoirsEnAttenteCorrection(devoirs) {
  return (devoirs || []).filter(
    (d) => d.type === "exercice" && d.statut === "fait" && d.reponseExercice && d.reponseExercice.note == null
  );
}

// Renvoie uniquement les devoirs "faits" dont la date de realisation se
// situe depuis le debut de la semaine derniere (samedi->vendredi). Les
// devoirs faits plus anciennement disparaissent de la liste affichee. Les
// exercices en attente de correction sont exclus d'ici : ils apparaissent
// dans leur propre section (voir filtrerDevoirsEnAttenteCorrection).
export function filtrerDevoirsFaitsRecents(devoirs, aujourdHui = new Date()) {
  const today = new Date(aujourdHui);
  today.setHours(0, 0, 0, 0);

  const debutSemaineEnCours = debutSemaine(today);
  const seuilSemaineDerniere = new Date(debutSemaineEnCours);
  seuilSemaineDerniere.setDate(seuilSemaineDerniere.getDate() - 7);

  return (devoirs || []).filter((d) => {
    if (d.statut !== "fait") return false;
    if (d.type === "exercice" && d.reponseExercice && d.reponseExercice.note == null) return false;
    if (!d.date_realisation) return false;
    const dateRealisation = new Date(d.date_realisation);
    return dateRealisation >= seuilSemaineDerniere;
  });
}

// Jalon "repertoire d'archivage" (signalement de Phil : les devoirs faits
// qui disparaissent de la liste principale ne sont pas vraiment archives
// nulle part, juste caches). Renvoie le complement exact de
// filtrerDevoirsFaitsRecents : les devoirs faits (hors exercices encore en
// attente de correction) dont la date de realisation est trop ancienne pour
// rester dans "Devoirs faits" — ils basculent automatiquement dans l'onglet
// Archives plutot que de disparaitre silencieusement.
export function filtrerDevoirsArchives(devoirs, aujourdHui = new Date()) {
  const today = new Date(aujourdHui);
  today.setHours(0, 0, 0, 0);

  const debutSemaineEnCours = debutSemaine(today);
  const seuilSemaineDerniere = new Date(debutSemaineEnCours);
  seuilSemaineDerniere.setDate(seuilSemaineDerniere.getDate() - 7);

  return (devoirs || []).filter((d) => {
    if (d.statut !== "fait") return false;
    if (d.type === "exercice" && d.reponseExercice && d.reponseExercice.note == null) return false;
    if (!d.date_realisation) return false;
    const dateRealisation = new Date(d.date_realisation);
    return dateRealisation < seuilSemaineDerniere;
  });
}

// Jalon "streak" (signalement de Phil : rendre l'application plus
// attractive pour une adolescente, inspire de Duolingo) : nombre de jours
// consecutifs, jusqu'a aujourd'hui, avec au moins un devoir marque fait. Un
// jour de grace est accorde pour aujourd'hui : si rien n'est encore fait
// aujourd'hui, le decompte part d'hier plutot que de casser le streak avant
// la fin de la journee.
function formaterJourLocal(date) {
  const annee = date.getFullYear();
  const mois = String(date.getMonth() + 1).padStart(2, "0");
  const jour = String(date.getDate()).padStart(2, "0");
  return `${annee}-${mois}-${jour}`;
}

export function calculerStreak(devoirs, aujourdHui = new Date()) {
  const joursFaits = new Set();
  for (const d of devoirs || []) {
    if (d.statut !== "fait" || !d.date_realisation) continue;
    joursFaits.add(formaterJourLocal(new Date(d.date_realisation)));
  }

  const cursor = new Date(aujourdHui);
  cursor.setHours(0, 0, 0, 0);
  if (!joursFaits.has(formaterJourLocal(cursor))) {
    cursor.setDate(cursor.getDate() - 1);
  }

  let streak = 0;
  while (joursFaits.has(formaterJourLocal(cursor))) {
    streak++;
    cursor.setDate(cursor.getDate() - 1);
  }
  return streak;
}

// Jalon "progression visible" (signalement de Phil : rendre l'application
// plus attractive pour une adolescente, comme le streak et le minuteur
// focus) : pourcentage de devoirs faits par matière, pour une barre de
// progression par matière (voir components/ProgressionMatieres.js). Utilise
// directement les champs matiereId/matiere/couleur déjà présents sur chaque
// devoir (voir lib/devoirsSupabase.js) — pas de requête supplémentaire.
// Une matière sans aucun devoir n'apparaît pas (rien à montrer).
export function progressionParMatiere(devoirs) {
  const parMatiere = new Map();

  for (const d of devoirs || []) {
    if (!d.matiereId) continue;
    if (!parMatiere.has(d.matiereId)) {
      parMatiere.set(d.matiereId, {
        matiereId: d.matiereId,
        nom: d.matiere || "",
        couleur: d.couleur || "#4169E1",
        total: 0,
        faits: 0,
      });
    }
    const entree = parMatiere.get(d.matiereId);
    entree.total += 1;
    if (d.statut === "fait") entree.faits += 1;
  }

  return Array.from(parMatiere.values())
    .map((e) => ({ ...e, pourcentage: e.total > 0 ? Math.round((e.faits / e.total) * 100) : 0 }))
    .sort((a, b) => a.nom.localeCompare(b.nom));
}
// Jalon "progression cumulee et ludique" (signalement de Phil : remplacer la
// barre de progression par matiere par une seule barre cumulee sur tous les
// devoirs, plus fun et attractive). Renvoie le nombre total de devoirs, le
// nombre de devoirs faits, et le pourcentage correspondant.
export function progressionGlobale(devoirs) {
  const total = (devoirs || []).length;
  const faits = (devoirs || []).filter((d) => d.statut === "fait").length;
  const pourcentage = total > 0 ? Math.round((faits / total) * 100) : 0;
  return { total, faits, pourcentage };
}
