...
import { chargerMessages, envoyerMessage, marquerCommeLu } from "@/lib/messagesSupabase";
...
export default function MessagerieFamille({ enfantId, compteId, titre }) {
  ...
  async function envoyer(e) {
    ...
    setErreur("");
    try {
      await envoyerMessage(enfantId, compteId, contenu);
      setTexte("");
      await recharger();
  ...
  return (
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col h-[28rem]">
      ...
      <form onSubmit={envoyer} className="border-t border-slate-200 dark:border-slate-700 p-3 flex items-end gap-2">
        <textarea
          value={texte}
