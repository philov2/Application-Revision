...
/* lectureSeule masque toutes les actions qui modifient les données (créer,
   ...
   modeArchive (Jalon "répértoire d'archivage", signalement de Phil) affiche
   cette même structure Matière > Chapitre > Documents mais pour les
   documents archivés (archive = true) plutôt que les documents actifs
   (archive = false) : pas de création de chapitre, pas d'import, pas de
   génération IA, pas de suppression de matière/chapitre — seulement ouvrir
...
export default function MatiereDocuments({ matiere, enfantId, compteId, lectureSeule = false, modeArchive = false, peutSupprimerMatiere = false, onMatiereSupprimee, onMatiereRenommee }) {
  ...
  // testsParChapitre ci-dessus, pour afficher et gérer les decks de
  // flashcards générés par IA au sein de chaque chapitre.
  const [flashcardsParChapitre, setFlashcardsParChapitre] = useState({});
  ...
  const [enConfirmationSuppressionFlashcards, setEnConfirmationSuppressionFlashcards] = useState(null);
  const [enCoursSuppressionFlashcards, setEnCoursSuppressionFlashcards] = useState(new Set());
  ...
  const [enConfirmationSuppressionMatiere, setEnConfirmationSuppressionMatiere] = useState(false);
  const [enCoursSuppressionMatiere, setEnCoursSuppressionMatiere] = useState(false);
  ...
  /* Renommage (corrige une erreur de frappe sans supprimer/recreer) —
     matiere et chapitres. */
  const [nomMatiereAffiche, setNomMatiereAffiche] = useState(matiere.nom);
  useEffect(() => { setNomMatiereAffiche(matiere.nom); }, [matiere.nom]);
  const [renommageMatiereOuvert, setRenommageMatiereOuvert] = useState(false);
  const [nomMatiereEdite, setNomMatiereEdite] = useState(matiere.nom);
  const [enCoursRenommageMatiere, setEnCoursRenommageMatiere] = useState(false);
  const [chapitreEnRenommage, setChapitreEnRenommage] = useState(null);
  const [nomChapitreEdite, setNomChapitreEdite] = useState("");
  const [enCoursRenommageChapitre, setEnCoursRenommageChapitre] = useState(new Set());
  ...
  async function charger() {
    ...
    if (chapitreIds.length > 0) {
      ...
      const { data: flashcards } = await supabase.from("flashcards").select("id, titre, chapitre_id").in("chapitre_id", chapitreIds);
      const flashcardsPourChapitre = {};
      (flashcards || []).forEach((f) => {
        if (!flashcardsPourChapitre[f.chapitre_id]) flashcardsPourChapitre[f.chapitre_id] = [];
        flashcardsPourChapitre[f.chapitre_id].push(f);
      });
      setFlashcardsParChapitre(flashcardsPourChapitre);
    } else {
      setTestsParChapitre({});
      setFlashcardsParChapitre({});
    }
  ...
  }, [matiere?.id, enfantId, modeArchive]);

  async function ajouterChapitre(e) {
    e.preventDefault();
    const nomSaisi = nouveauChapitre.trim();
    if (!nomSaisi) return;
    if (chapitres.some((c) => c.nom.trim().toLowerCase() === nomSaisi.toLowerCase())) {
      setMessage("Ce chapitre existe déjà dans cette matière.");
      return;
    }
    const { error } = await supabase.from("chapitres").insert({ matiere_id: matiere.id, nom: nomSaisi });
    if (error) {
  ...
  /* Supprime la matiere entiere (referentiel commun, reserve a l'admin et
     aux parents -- voir app/api/matieres/[id]/route.js). La route bloque la
     suppression si des chapitres, documents ou devoirs existent encore pour
     cette matiere, pour ne jamais faire disparaitre un historique de devoirs
     sans confirmation explicite. */
  async function supprimerMatiere() {
    setEnCoursSuppressionMatiere(true);
    setMessage("");
    try {
      await authFetch(`/api/matieres/${matiere.id}`, { method: "DELETE" });
      setEnConfirmationSuppressionMatiere(false);
      onMatiereSupprimee?.(matiere.id);
    } catch (err) {
      setMessage(err.message);
      setEnCoursSuppressionMatiere(false);
    }
  }

  /* Renomme la matiere (corrige une erreur de frappe) sans avoir a la
     supprimer puis la recreer, ce qui ferait perdre son historique de
     chapitres/documents/devoirs. Ouvert aux memes roles que la creation
     (parent, soutien, admin) -- voir app/api/matieres/[id]/route.js. */
  async function renommerMatiere(e) {
    e.preventDefault();
    const nom = nomMatiereEdite.trim();
    if (!nom) return;
    if (nom === nomMatiereAffiche) {
      setRenommageMatiereOuvert(false);
      return;
    }
    setEnCoursRenommageMatiere(true);
    setMessage("");
    try {
      await authFetch(`/api/matieres/${matiere.id}`, { method: "PATCH", body: JSON.stringify({ nom }) });
      setNomMatiereAffiche(nom);
      setRenommageMatiereOuvert(false);
      onMatiereRenommee?.(matiere.id, nom);
    } catch (err) {
      setMessage(err.message);
    } finally {
      setEnCoursRenommageMatiere(false);
    }
  }

  /* Renomme un chapitre (meme principe que renommerMatiere ci-dessus). */
  function ouvrirRenommageChapitre(c) {
    setNomChapitreEdite(c.nom);
    setChapitreEnRenommage(c.id);
  }

  async function renommerChapitre(e, chapitreId) {
    e.preventDefault();
    const nom = nomChapitreEdite.trim();
    if (!nom) return;
    setEnCoursRenommageChapitre((prev) => new Set(prev).add(chapitreId));
    setMessage("");
    try {
      await authFetch(`/api/chapitres/${chapitreId}`, { method: "PATCH", body: JSON.stringify({ nom }) });
      setChapitreEnRenommage(null);
      charger();
    } catch (err) {
      setMessage(err.message);
    } finally {
      setEnCoursRenommageChapitre((prev) => {
        const next = new Set(prev);
        next.delete(chapitreId);
        return next;
      });
  ...
  /* Crée automatiquement un devoir pour l'enfant après une génération IA
     réussie (synthèse, exercices, test ou flashcards), pour que le nouveau
     contenu n'existe pas seulement dans l'onglet Chapitres et documents mais
  ...
  async function creerDevoirAuto({ type, chapitreId, documentId, testId, flashcardsId }) {
    try {
      await creerDevoir({
        enfantId,
        ...
        testId: testId || null,
        flashcardsId: flashcardsId || null,
        titre: null,
        type,
  ...
  async function genererTestIA(d) {
    ...
    setMessage("");
    try {
      const resultat = await authFetch(`/api/documents/${d.id}/test-ia`, { method: "POST" });
      ...
      await creerDevoirAuto({ type: "test", chapitreId: d.chapitre_id, documentId: d.id, testId: resultat?.test?.id });
      charger();
  ...
  /* Jalon "flashcards" (signalement de Phil) : même principe que
     genererTestIA ci-dessus — les flashcards ne sont pas non plus un
     "document" (table flashcards à part), donc le devoir créé automatiquement
     est de type "revision" (comme la synthèse) mais référence le deck de
     flashcards via flashcardsId plutôt qu'un document. */
  async function genererFlashcardsIA(d) {
    setEnCoursFlashcardsIA((prev) => new Set(prev).add(d.id));
    setMessage("");
    try {
      const resultat = await authFetch(`/api/documents/${d.id}/flashcards-ia`, { method: "POST" });
      await creerDevoirAuto({ type: "revision", chapitreId: d.chapitre_id, flashcardsId: resultat?.flashcards?.id });
      charger();
      setMessage("Flashcards générées par IA et ajoutées aux devoirs de l'enfant.");
    } catch (err) {
      setMessage(err.message);
    } finally {
      setEnCoursFlashcardsIA((prev) => {
        const next = new Set(prev);
        next.delete(d.id);
        return next;
      });
  ...
  /* Jalon "répertoire d'archivage" (signalement de Phil) : archive un
     document sans le supprimer — il disparait de cet onglet Documents
     (archive = false) et réapparait dans l'onglet Archives (archive = true),
     groupé de la même façon par Matière > Chapitre. Aucune suppression n'a
     lieu ici ; c'est uniquement depuis l'onglet Archives que la suppression
     devient définitive. */
  async function archiverDocument(documentId) {
    setEnCoursArchivage((prev) => new Set(prev).add(documentId));
    setMessage("");
    try {
      await authFetch(`/api/documents/${documentId}`, { method: "PATCH", body: JSON.stringify({ archive: true }) });
      charger();
    } catch (err) {
      setMessage(err.message);
    } finally {
      setEnCoursArchivage((prev) => {
        const next = new Set(prev);
        next.delete(documentId);
        return next;
      });
  ...
  async function retirerFlashcards(flashcardsId) {
    setEnCoursSuppressionFlashcards((prev) => new Set(prev).add(flashcardsId));
    setMessage("");
    try {
      await supprimerFlashcards(flashcardsId);
      setEnConfirmationSuppressionFlashcards(null);
      charger();
    } catch (err) {
      setMessage(err.message);
    } finally {
      setEnCoursSuppressionFlashcards((prev) => {
        const next = new Set(prev);
        next.delete(flashcardsId);
        return next;
      });
    }
  }
  ...
  /* En mode archive, on ne montre que les chapitres qui contiennent
     effectivement au moins un document archivé — sinon l'onglet Archives
     afficherait tous les chapitres de la matière, vides pour la plupart. */
  const chapitresAffiches = modeArchive ? chapitres.filter((c) => (documentsParChapitre[c.id] || []).length > 0) : chapitres;

  /* Une matière sans aucun document archivé n'a rien à montrer dans l'onglet
     Archives : la carte ne s'affiche pas du tout, plutôt que d'afficher une
     matière vide. */
  if (modeArchive && documents.length === 0) return null;
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {!lectureSeule && !modeArchive && d.type === "cours" && (
            <>
              ...
              <button
                onClick={() => genererFlashcardsIA(d)}
                disabled={enCoursFlashcardsIA.has(d.id) || !d.chapitre_id}
                title={!d.chapitre_id ? "Rattachez d'abord ce document à un chapitre" : "Génère des flashcards et crée automatiquement un devoir pour l'enfant"}
                className={PILL_IA}
              >
          ...
          {!lectureSeule && !modeArchive && (
            <button onClick={() => archiverDocument(d.id)} disabled={enCoursArchivage.has(d.id)} className={PILL_NEUTRE} title="Archiver ce document (le retire de l'onglet Documents sans le supprimer)">
              {enCoursArchivage.has(d.id) ? "..." : "🗄 Archiver"}
            </button>
          )}
          {!lectureSeule && (
            enConfirmationSuppression === d.id ? (
              ...
              <button onClick={() => setEnConfirmationSuppression(d.id)} className={PILL_DANGER}>🗑 {modeArchive ? "Suppr. définitivement" : "Suppr."}</button>
            )
          )}
        </div>
      </div>
    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 p-4 space-y-4" style={{ borderLeft: `6px solid ${matiere.couleur}` }}>
      <div className="w-full flex items-center justify-between gap-2">
        ...
        <span className="text-xs text-slate-400 shrink-0">
          {chapitresAffiches.length} chapitre{chapitresAffiches.length !== 1 ? "s" : ""} · {documents.length} document{documents.length !== 1 ? "s" : ""}
        </span>
        {!lectureSeule && !modeArchive && !renommageMatiereOuvert && (
          <button type="button" onClick={() => { setNomMatiereEdite(nomMatiereAffiche); setRenommageMatiereOuvert(true); }} className={`${PILL_NEUTRE} shrink-0`} title="Renommer la matière">✎</button>
        )}
        {peutSupprimerMatiere && !modeArchive && (
          enConfirmationSuppressionMatiere ? (
            <span className="flex items-center gap-1.5 shrink-0">
              <span className={PILL_AVERTISSEMENT}>Sûr ?</span>
              <button onClick={supprimerMatiere} disabled={enCoursSuppressionMatiere} className={PILL_DANGER_SOLIDE}>
                {enCoursSuppressionMatiere ? "..." : "Oui"}
              </button>
              <button onClick={() => setEnConfirmationSuppressionMatiere(false)} className={PILL_NEUTRE}>Annuler</button>
            </span>
          ) : (
            <button onClick={() => setEnConfirmationSuppressionMatiere(true)} className={`${PILL_DANGER} shrink-0`}>🗑 Suppr. matière</button>
          )
        )}
      </div>
        ...
        <>
          ...
          {chapitresAffiches.length === 0 && (
            <p className="text-slate-400 text-xs">
              {modeArchive
                ? "Aucun document archivé dans cette matière."
                : lectureSeule
                ...
                <div key={c.id} className="rounded-lg border-2 border-slate-200 dark:border-slate-600 p-3 space-y-2.5 bg-slate-50/60 dark:bg-slate-800/20">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    {chapitreEnRenommage === c.id ? (
                      <form onSubmit={(e) => renommerChapitre(e, c.id)} className="flex items-center gap-1.5 min-w-0 flex-1">
                        <input value={nomChapitreEdite} onChange={(e) => setNomChapitreEdite(e.target.value)} autoFocus className="flex-1 min-w-0 rounded-lg border border-slate-300 dark:border-slate-600 bg-transparent px-2 py-1 text-xs" />
                        <button type="submit" disabled={enCoursRenommageChapitre.has(c.id) || !nomChapitreEdite.trim()} className={PILL_NEUTRE}>{enCoursRenommageChapitre.has(c.id) ? "..." : "✓ Ok"}</button>
                        <button type="button" onClick={() => setChapitreEnRenommage(null)} className={PILL_NEUTRE}>Annuler</button>
                      </form>
                    ) : (
                      <button type="button" onClick={() => toggleChapitre(c.id)} className="flex items-center gap-1.5 text-left min-w-0">
                        ...
                        <span className="text-xs text-slate-400 shrink-0">
                          ...
                          {flashcardsChapitre.length > 0 ? `, ${flashcardsChapitre.length} flashcards${flashcardsChapitre.length !== 1 ? "" : ""}` : ""})
                        </span>
                      </button>
                    )}
                    {!lectureSeule && !modeArchive && chapitreEnRenommage !== c.id && (
                      <div className="flex items-center gap-1.5">
                        <button onClick={() => ouvrirRenommageChapitre(c)} className={PILL_NEUTRE} title="Renommer le chapitre">✎</button>
                  ...
                  {!estReduit && (
                    <>
                      ...
                      {!modeArchive && testsChapitre.length > 0 && (
                      ...
                      {!modeArchive && flashcardsChapitre.length > 0 && (
                        <div className="space-y-1 pl-1">
                          {flashcardsChapitre.map((f) => (
                            <div key={f.id} className="flex items-center justify-between text-xs">
                              <span>🗂 {f.titre}</span>
                              {!lectureSeule && (
                                enConfirmationSuppressionFlashcards === f.id ? (
                                  <span className="flex items-center gap-1.5">
                                    <span className={PILL_AVERTISSEMENT}>Sûr ?</span>
                                    <button onClick={() => retirerFlashcards(f.id)} disabled={enCoursSuppressionFlashcards.has(f.id)} className={PILL_DANGER_SOLIDE}>
                                      {enCoursSuppressionFlashcards.has(f.id) ? "..." : "Oui"}
                                    </button>
                                    <button onClick={() => setEnConfirmationSuppressionFlashcards(null)} className={PILL_NEUTRE}>Annuler</button>
                                  </span>
                                ) : (
                                  <button onClick={() => setEnConfirmationSuppressionFlashcards(f.id)} className={PILL_DANGER}>🗑 Suppr.</button>
                                )
                              )}
                            </div>
                          ))}
                      ...
                      <div className="space-y-1.5 pl-1">
                        ...
                        {docsChapitre.length === 0 && testsChapitre.length === 0 && flashcardsChapitre.length === 0 && (
              ...
              <div className="space-y-1.5">
                {documentsSansChapitre.map((d) => (
                  <div key={d.id} className="flex items-center justify-between text-sm rounded-lg border border-slate-200 dark:border-slate-700 px-3 py-2 flex-wrap gap-2 bg-white dark:bg-slate-900/40">
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {chapitres.length > 0 && (
                        <select disabled={enCoursAssignation.has(d.id)} onChange={(e) => assignerChapitre(d.id, e.target.value)} defaultValue="" className="rounded-lg border border-slate-300 dark:border-slate-600 bg-transparent px-2 py-1 text-xs">
          ...
          {!lectureSeule && modeArchive && documentsSansChapitre.length > 0 && (
            <div className="space-y-1.5">
              {documentsSansChapitre.map((d) => <DocumentRow key={d.id} d={d} />)}
            </div>
          )}
