      ...
      try {
        ...
        if (!annule) {
          if (nonLusRef.current !== null && n > nonLusRef.current) {
            envoyerNotification("Nouveau message", `Vous avez un nouveau message — ${nomEnfant}.`);
          }
          nonLusRef.current = n;
          setNonLus(n);
        }
    ...
    <>
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto px-4 py-8 space-y-8">
          ...
          <>
            ...
            <section>
              <div className="flex items-center justify-between mb-3">
                <h2 className="font-semibold">Devoirs de {nomEnfant}</h2>
              ...
              <div className="space-y-3">
                ...
                {devoirsAFaireTries.length === 0 && <p className="text-slate-500 dark:text-slate-400 text-sm">Aucun devoir pour ces matières.</p>}
              </div>
            </section>
        ...
        {onglet === "documents" && enfantId && compteId && (
          <section>
            ...
            {matieres.length > 0 ? (
              ...
              <p className="text-slate-500 dark:text-slate-400 text-xs">Aucune matière ne vous est encore confiée.</p>
            )}
          </section>
        )}
        ...
        {onglet === "messages" && enfantId && compteId && (
          <section>
            <MessagerieFamille enfantId={enfantId} compteId={compteId} titre={`Messages — ${nomEnfant}`} />
          </section>
        )}
      </main>
    </>
