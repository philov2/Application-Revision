...
function Contenu() {
  ...
  const [formCoparentOuvert, setFormCoparentOuvert] = useState(false);
  ...
  /* Jalon "onglet Gestion du compte" (signalement de Phil : regrouper nouvel */
  /* enfant, nouveau soutien et invitation co-parent dans un onglet dedie, */
  /* separe des devoirs/documents/messages, pour simplifier l'ecran). */
  /* Jalon "repertoire d'archivage" (signalement de Phil : les devoirs faits */
  /* trop anciens pour rester dans l'onglet Devoirs, et les documents */
  /* archives manuellement depuis l'onglet Documents, se retrouvent dans un */
  /* onglet dedie plutot que de disparaitre). Place en dernier dans la barre */
  /* d'onglets (signalement de Phil), apres Gestion du compte. */
  const [onglet, setOnglet] = useState("devoirs"); /* "devoirs" | "documents" | "messages" | "compte" | "archives" */
  ...
  useEffect(() => {
    if (!supabaseConfigured || !enfant?.id || !compteId) {
      setNonLus(0);
      return;
    }
    let annule = false;
    ...
    async function rafraichirNonLus() {
      try {
        const n = await compterNonLus(enfant.id, compteId);
        if (!annule) {
          if (nonLusRef.current !== null && n > nonLusRef.current) {
            envoyerNotification("Nouveau message", `Vous avez un nouveau message — ${enfant.nom}.`);
          }
          nonLusRef.current = n;
          setNonLus(n);
        }
  ...
  /* Jalon "compte co-parent" (signalement de Phil : deux parents distincts */
  /* doivent pouvoir suivre le meme enfant) : demande soumise a validation par */
  /* l'administrateur, comme pour un soutien. A la validation, le nouveau */
  /* compte est rattache automatiquement a TOUS les enfants du demandeur (voir */
  /* app/api/demandes/[id]/valider/route.js). */
  async function demanderCoparent(e) {
    e.preventDefault();
    setMessage("");
    const form = new FormData(e.target);
    try {
  ...
  async function creerNouvelleMatiere(e) {
    ...
    setMessage("");
    if (matieres.some((m) => m.nom.trim().toLowerCase() === nomMatiereSaisi.toLowerCase())) {
      setMessage("Cette matière existe déjà.");
      return;
    }
    setEnCoursMatiere(true);
    try {
      const couleur = choisirCouleurMatiere(nomMatiereSaisi, matieres);
      const { data, error } = await supabase.from("matieres").insert({ nom: nomMatiereSaisi, couleur }).select().single();
      if (error) throw error;
  ...
  function retirerMatiereLocale(id) {
    setMatieres((prev) => prev.filter((m) => m.id !== id));
  }

  /* Renommage d'une matiere (corrige une erreur de frappe) : met a jour la
     liste locale sans recharger toute la page, comme retirerMatiereLocale
     ci-dessus pour la suppression. */
  function renommerMatiereLocale(id, nom) {
  ...
  const devoirsArchivesListe = filtrerDevoirsArchives(devoirs);
  ...
  return (
    <>
      ...
      <main className="flex-1 max-w-3xl w-full mx-auto px-4 py-8 space-y-6">
        ...
        {onglet === "messages" && enfant && compteId && (
          <section>
            <MessagerieFamille enfantId={enfant.id} compteId={compteId} titre={`Messages — ${enfant.nom}`} />
          </section>
        )}
          ...
          <>
            <section>
              ...
              {enfants.length === 0 && !formEnfantOuvert && (
                <p className="text-slate-500 dark:text-slate-400 text-xs">Aucun enfant pour l&apos;instant.</p>
              )}
            </section>
